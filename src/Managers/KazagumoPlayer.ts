...
import { KazagumoError, KazagumoPlayerOptions, PlayerState, Events, PlayOptions, KazagumoSearchOptions, KazagumoSearchResult } from "../Modules/Interfaces";
...
export default class KazagumoPlayer {
    ...
    constructor(kazagumo: Kazagumo, player: Player, options: KazagumoPlayerOptions) {
        ...
        this.shoukaku.on("end", (data) => {
            ...
            if (this.state === PlayerState.DESTROYING || this.state === PlayerState.DESTROYED) return this.emit(Events.Debug, `Player ${this.guildId} destroyed from end event`);
    ...
    public async play(track?: KazagumoTrack, options?: PlayOptions | undefined): Promise<KazagumoPlayer> {
        ...
        let playOptions = { track: current.track, options: {} };
    ...
    public connect(): KazagumoPlayer {
        if (this.state === PlayerState.DESTROYED) throw new KazagumoError(1, "Player is already destroyed");
        if (this.state === PlayerState.CONNECTED || !!this.voiceId) throw new KazagumoError(1, "Player is already connected");
        this.state = PlayerState.CONNECTING;
    ...
    public disconnect(): KazagumoPlayer {
        if (this.state === PlayerState.DISCONNECTED || !this.voiceId) throw new KazagumoError(1, "Player is already disconnected");
    ...
    destroy(): KazagumoPlayer {
        if (this.state === PlayerState.DESTROYING || this.state === PlayerState.DESTROYED) throw new KazagumoError(1, "Player is already destroyed");
