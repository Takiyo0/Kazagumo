...
import { rawTrack, SupportedSources, SourceIDs, KazagumoError, escapeRegExp, ResolveOptions, Events } from "../../Modules/Interfaces";
...
export default class KazagumoTrack {
    ...
    constructor(rawTrack: rawTrack, requester: unknown) {
        this.kazagumo = undefined;

        this.track = rawTrack.track;
        this.sourceName = rawTrack.info.sourceName;
        this.title = rawTrack.info.title;
        this.uri = rawTrack.info.uri;
        this.identifier = rawTrack.info.identifier;
        this.isSeekable = rawTrack.info.isSeekable;
        this.isStream = rawTrack.info.isStream;
        this.author = rawTrack.info.author;
        this.length = rawTrack.info.length;
        this.position = rawTrack.info.position;
        this.thumbnail = rawTrack.info.thumbnail;
        this.realUri = SupportedSources.includes(this.sourceName) ? this.uri : null;

        this.requester = requester;

        if (this.sourceName === "youtube" && this.identifier) this.thumbnail = `https://img.youtube.com/vi/${this.identifier}/hqdefault.jpg`;
    }

    setKazagumo(kazagumo: Kazagumo): KazagumoTrack {
        this.kazagumo = kazagumo;
        if (this.sourceName === "youtube" && this.identifier) this.thumbnail = `https://img.youtube.com/vi/${this.identifier}/${kazagumo.KazagumoOptions.defaultYoutubeThumbnail ?? "hqdefault"}.jpg`;
    ...
    private async getTrack(): Promise<Track> {
        ...
        if (this.length) {
            const sameDuration = result.tracks.find(track => (track.info.length >= ((this.length ? this.length : 0) - 2000)) && (track.info.length <= ((this.length ? this.length : 0) + 2000)));
