...
export class KazagumoQueue extends Array<KazagumoTrack> {
    ...
    public add(track: KazagumoTrack | KazagumoTrack[]): KazagumoQueue {
        if (Array.isArray(track) && track.some(t => !(t instanceof KazagumoTrack))) throw new KazagumoError(1, "Track must be an instance of KazagumoTrack");
    ...
    public remove(position: number): KazagumoQueue {
        if (position < 0 || position >= this.length) throw new KazagumoError(1, "Position must be between 0 and " + (this.length - 1));
